...
class LogicWizard(QWizard,Ui_Wizard):
    ...
    def Select_JiaZhiLianCheng_Shibor_Rate_Trades(df_onenight_shibor,df_trades,save_path):
        ...
        df_trades['起息日'] = df_trades['起息日'].astype('datetime64[ns]')
    ...
    def Select_Trades_By_Cumsum(self,df10,save_path):
        ...
        df10['修正后的交易量'] = df10.apply(lambda x: Adjust_Trade_Vol(x['首期结算金额'], x['累计加总额度'], x['是否大于阀值的累计和'], pool_vol),
                                     axis=1)
        df11 = df10[(df10['是否大于阀值的累计和'] < 2) & (df10['起息日（间隔日大于1日的有调整）'] >= pd.to_datetime(self.dateEdit.text())) & (df10['起息日（间隔日大于1日的有调整）'] <= pd.to_datetime(self.dateEdit_2.text()))]
        save_path = os.path.join(save_path, "[11]根据统计日期的范围提取最终被选出的交易.xlsx")
        df11.to_excel(save_path, index=False)
        print("完成>>>>[11]根据统计日期的范围提取最终被选出的交易.xlsx")
        return df11

    def Calc_Interest(self, df11,save_path):
        pool_vol = int(self.lineEdit_3.text()) * 100000000
        #df11 = pd.read_excel(r"D:\snpythonproject\JZLC2020V1\[11]根据统计日期的范围提取最终被选出的交易.xlsx")
        ...
        save_path = os.path.join(save_path, "[12]根据统计日期的范围提取最终被选出的交易_计算结果.xlsx")

        writer = pd.ExcelWriter(save_path)
        df12.to_excel(writer, sheet_name='最终结果')
        writer.save()
        print("完成>>>>[12]根据统计日期的范围提取最终被选出的交易_计算结果.xlsx")

        #
        # df12.to_excel(save_path, engine = 'openpyxl')
        # print("完成>>>>[12]根据统计日期的范围提取最终被选出的交易_计算结果.xlsx")
        return total_interest,profit_interest_rate,df_nostyle



    def merge_same_trade(self, df12,save_path):
        '''
        df12中存在这样一种情况，实质是一笔非隔夜N天的交易，为了算每天的收益，将该笔交易拆分成了N笔
        该函数的目的是将被拆分后的N笔交易，再次归为一笔交易
        :param df12:
        :param save_path:
        :return:
        '''
        df12['累计加总收益'] = df12.groupby('外部成交编号')['当日收益'].cumsum()
        df12 = df12[['交易号','外部成交编号','交易对手','是否为价值连城客户','买卖方向','回购利率','首期结算金额','起息日','到期日','累计加总收益']]
        df13 = df12.sort_values(["外部成交编号", "累计加总收益"])
        df13 = df13.drop_duplicates("外部成交编号", keep='last')
        df13.sort_values(by=['起息日', '回购利率'], inplace=True, ascending=[True, False])
        save_path = os.path.join(save_path, "[13](将同一笔交易合并)根据统计日期的范围提取最终被选出的交易_计算结果.xlsx")
        df13.to_excel(save_path, index=False)
        print("完成>>>>[13](将同一笔交易合并)根据统计日期的范围提取最终被选出的交易_计算结果.xlsx")
    ...
    def run(self):
        ...
        self.merge_same_trade(profit_result[2], file_save_path)
